...
import { machineData, MachineSpec } from '../data/machineData';
...
export default function ProcessSheet() {
  ...
  const [currentMachine, setCurrentMachine] = useState<MachineSpec | null>(null);
  ...
  const handleMachineChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    let found: MachineSpec | null = null;
    for (const brand of machineData) {
        const m = brand.machines.find(m => m.series === val);
        if (m) { found = m; break; }
    }
    setCurrentMachine(found);
  };
  ...
  const handleMaterialChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (mat) {
      ...
      setInputs(prev => ({
        ...
        ejectTemp: parseRange(mat.moldTemp) + 40,
        matCost: mat.costPerKg,
        dryerTemp: parseInt(mat.drying.split('°')[0]) || 80
      }));
    }
  };
  ...
  useEffect(() => {
    ...
    const effectiveCooling = Math.max(coolTime, inputs.timeCooling); 
    const totalCycle = inputs.timeInjection + effectiveCooling + inputs.timeMoldMove + inputs.timeEject + (inputs.robotUsed ? 1.5 : 0);

    // 4. Machine Dependant
    const screwDia = currentMachine ? currentMachine.screwDiameter : 40;
    const barrelCap = currentMachine ? currentMachine.maxShotVolume * inputs.density : 1000;
    ...
    // Cushion (5-15% of max volume)
    const cushionMin = currentMachine ? currentMachine.maxShotVolume * 0.05 : 0;
    const cushionMax = currentMachine ? currentMachine.maxShotVolume * 0.15 : 0;
        ...
        <div className="flex flex-col xl:flex-row gap-6 flex-1 overflow-hidden print:block">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="card-input border-blue-500/30">
                        ...
                        <div className="space-y-4">
                            <select onChange={handleMachineChange} className="input-field-alt print:bg-white print:text-black">
                                <option value="">-- {t('select_machine')} --</option>
                                {machineData.map(brand => (
                                    <optgroup key={brand.id} label={brand.name}>
                                        {brand.machines.map(m => (
                                            <option key={m.series} value={m.series}>{m.series}</option>
                                        ))}
                                    </optgroup>
                                ))}
                            </select>
                            {currentMachine && (
                                <div className="grid grid-cols-2 gap-x-4 gap-y-2 py-2 border-t border-slate-800 mt-2">
                                    <LimitSpec label="Max Clamp" val={`${currentMachine.clampForce} kN`} />
                                    <LimitSpec label="Max Shot" val={`${currentMachine.maxShotVolume} cm³`} />
                                    <LimitSpec label="Tie Bars" val={currentMachine.tieBarSpacing} />
                    ...
                    <div className="card-input border-orange-500/30">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <InputGroup label="Area (cm²)" value={inputs.projectedArea} onChange={(v: string) => handleInput('projectedArea', v)} />
                            <InputGroup label="Steel" type="text" value={inputs.moldSteel} onChange={(v: string) => handleStrInput('moldSteel', v)} />
                            <InputGroup label="Safety Factor" value={inputs.safetyFactor} onChange={(v: string) => handleInput('safetyFactor', v)} step="0.1" />
                        </div>
                    </div>
                </div>

                {/* SECTION 2: MATERIAL & PROCESS */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="card-input border-purple-500/30">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <InputGroup label="Wall (mm)" value={inputs.wallThickness} onChange={(v: string) => handleInput('wallThickness', v)} step="0.1" />
                                <InputGroup label="Flow (mm)" value={inputs.flowLength} onChange={(v: string) => handleInput('flowLength', v)} />
                            </div>
                        </div>
                    </div>

                    <div className="card-input border-green-500/30">
                        <h3 className="section-title text-green-400"><Clock size={16}/> {t('cycle_breakdown')} (s)</h3>
                        <div className="grid grid-cols-2 gap-4">
                            <InputGroup label="Injection" value={inputs.timeInjection} onChange={(v: string) => handleInput('timeInjection', v)} step="0.1" />
                            <InputGroup label="Cooling (Set)" value={inputs.timeCooling} onChange={(v: string) => handleInput('timeCooling', v)} placeholder={results.coolingTimeCalc.toFixed(1)} />
                            <InputGroup label="Mold Move" value={inputs.timeMoldMove} onChange={(v: string) => handleInput('timeMoldMove', v)} step="0.1" />
                            <InputGroup label="Ejection" value={inputs.timeEject} onChange={(v: string) => handleInput('timeEject', v)} step="0.1" />
                            <div className="col-span-2 pt-2 border-t border-slate-800 flex justify-between items-center">
                                <span className="text-slate-500 text-[10px] uppercase font-bold">Calculated Min Cooling</span>
                                <span className="text-blue-400 font-mono text-sm font-bold">{results.coolingTimeCalc.toFixed(1)} s</span>
                            </div>
                        </div>
                    </div>
                </div>

                {/* SECTION 3: COSTING & QUOTATION */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="card-input border-yellow-500/30">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <InputGroup label="Mach. €/h" value={inputs.machineRate} onChange={(v: string) => handleInput('machineRate', v)} />
                            <InputGroup label="Labor €/h" value={inputs.laborRate} onChange={(v: string) => handleInput('laborRate', v)} />
                            <InputGroup label="Scrap %" value={inputs.scrapRate} onChange={(v: string) => handleInput('scrapRate', v)} />
                            <InputGroup label="Mat. €/kg" value={inputs.matCost} onChange={(v: string) => handleInput('matCost', v)} />
                        </div>
                    </div>
                    ...
                    <div className="card-input border-pink-500/30">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <InputGroup label="Annual Vol" value={inputs.annualVolume} onChange={(v: string) => handleInput('annualVolume', v)} />
                            <InputGroup label="Discount %" value={inputs.volumeDiscount} onChange={(v: string) => handleInput('volumeDiscount', v)} />
                            <InputGroup label="Tool Invest €" value={inputs.toolInvestment} onChange={(v: string) => handleInput('toolInvestment', v)} />
                        </div>
                    </div>
                </div>

                {/* SECTION 4: AUXILIARIES */}
                <div className="card-input border-slate-500/30">
                    <h3 className="section-title text-slate-400"><Settings size={16}/> {t('auxiliaries')}</h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        ...
                        <InputGroup label={t('dryer_temp')} value={inputs.dryerTemp} onChange={(v: string) => handleInput('dryerTemp', v)} />
                        <InputGroup label="Temp Controllers" value={inputs.tempControllers} onChange={(v: string) => handleInput('tempControllers', v)} />
                        <div className="flex items-center gap-3">
                            <input type="checkbox" defaultChecked className="w-5 h-5 rounded bg-slate-950 border-slate-700" />
                            <label className="lbl mb-0">{t('conveyor')}</label>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="w-full xl:w-[400px] flex flex-col gap-6 shrink-0 print:w-full print:mt-8">
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6 shadow-2xl flex-1 overflow-y-auto custom-scrollbar print:overflow-visible">
                    ...
                    <ValidationCard label="Clamp Force" val={results.clampingForce} limit={currentMachine?.clampForce || 0} unit="kN" />
                    <ValidationCard label="Shot Volume" val={results.shotVolumeTotal} limit={currentMachine?.maxShotVolume || 0} unit="cm³" />
...
const InputGroup = ({ label, value, onChange, type = "number", step = "1", placeholder = "" }: any) => (
    <div>
...
const ValidationCard = ({ label, val, limit, unit }: any) => {
    const isExceeded = limit > 0 && val > limit;
    const isClose = limit > 0 && val > limit * 0.9 && val <= limit;
    ...
    return (
        <div className={`p-4 rounded-xl border ${isExceeded ? 'bg-red-500/5 border-red-500/30' : isClose ? 'bg-yellow-500/5 border-yellow-500/30' : 'bg-slate-950/50 border-slate-800'}`}>
            <div className="flex justify-between items-center mb-2">
                ...
                {limit > 0 && <span className="text-[10px] text-slate-600 font-mono">Limit: {limit} {unit}</span>}
            </div>
